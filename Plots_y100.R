libs <- c('lattice','gtable','hexbin','ggplot2','corrplot', 'latticeExtra', 'gridExtra', 'MASS', 
          'colorspace', 'plyr', 'Hmisc', 'scales','ggthemes','boot','lmom')
lapply(libs, require, character.only = T)

######################################################
################################# shift listko_y100 // a6142200 is for y100 shiftst[2] and for y60 shiftst[15]
listko<-listko_y100
shift<-shift_30
shiftst<-shiftst_30
shiftst<-shiftst_40
...
# only one river
xyplot(shiftst[[2]][,1]+shiftst[[2]][,2]+shiftst[[2]][,3]+shiftst[[2]][,4]~c(1908:1977),type=c("l","smooth"),
       col.line=c("black","red","darkgreen","blue"),xlab="Years",ylab="Standardized Discharge",grid=TRUE)

# all rivers separately -> get reg_df first!!!
...
shiftst[[1]]
############################### Correlation Matrix
################### Correlation 45 Rivers, shift 30 years. get correlation first
#Weibull
shkw<-matrix(NA,69,45)
for (i in seq(1,45,1)){
  for (j in seq(1,69,1)){
...
rownames(shkw)<-seq(1908,1976,1)
shkwt<-t(shkw)
reswt<-cor(shkwt,shkwt)
resw<-cor(shkw,shkw)

#Gumbel
shkg<-matrix(NA,69,45)
for (i in seq(1,45,1)){
  for (j in seq(1,60,1)){
...
colnames(shkg)<-dput(names(shift[1:45]))
rownames(shkg)<-seq(1908,1976,1)
shkgt<-t(shkg)
resgt<-cor(shkgt,shkgt)
resg<-cor(shkg,shkg)

#Pe3
shkp<-matrix(NA,69,45)
for (i in seq(1,45,1)){
  for (j in seq(1,69,1)){
...
rownames(shkp)<-seq(1908,1976,1)
shkpt<-t(shkp)
respt<-cor(shkpt,shkpt)
resp<-cor(shkp,shkp)

shkl<-matrix(NA,69,45)
for (i in seq(1,45,1)){
  for (j in seq(1,69,1)){
...
rownames(shkl)<-seq(1908,1976,1)
shklt<-t(shkl)
reslt<-cor(shklt,shklt)

############################### Regions corrplot for y100 // 30 years
### plot all regions
resw_plot<-corrplot(resw,order="hclust",addrect=5,cl.ratio = 0.1, cl.align = "r",tl.cex=0.5)
comp<-dput(maa_y100[,1])
plot(resw_plot)
corrplot(reswt,order="hclust",addrect=5,cl.ratio = 0.1, cl.align = "r",tl.cex=0.5)
#### find regions via k-means


## Region1 6943100:6731450; 1:16 get values and coordinates
region1<-matrix(NA,16,16)
colnames(region1)<-dput(colnames(resw_plot)[1:16])
rownames(region1)<-dput(rownames(resw_plot)[1:16])
for (i in seq(1:16)){
  for(j in seq(1:16)){
...
reg1_cor_y100<-matrix(NA,16,3)
for (i in seq(1:16)){
  for (j in seq(1:45))
    if(colnames(region1)[i]==comp[j]){
      reg1_cor_y100[i,]<-maa_y100[j,]
    }
}
...
for (i in seq(1:16)){
  for (j in seq(1:45))
    if(colnames(region1)[i]==comp[j]){
      shiftforcor1[i]<-shiftst_30[j]
      names(shiftforcor1)[i]<-names(shiftst_30)[j]
    }
}
shkw_r1<-matrix(NA,70,16)
for (i in seq(1,16,1)){
  for (j in seq(1,70,1)){
...
rownames(shkw_r1)<-seq(1908,1976,1)
shkwt_r1<-t(shkw_r1)
...
######region2 /// 17:23 6335301:6435060
region2<-matrix(NA,7,7)
colnames(region2)<-dput(colnames(resw_plot)[17:23])
rownames(region2)<-dput(rownames(resw_plot)[17:23])
for (i in seq(1:7)){
  for(j in seq(1:7)){
    region2[i,j]<-resw_plot[(i+16),(j+16)]
    
  }
}
reg2_cor_y60<-matrix(NA,7,3)
for (i in seq(1:7)){
  for (j in seq(1:45))
...
for (i in seq(1:7)){
  for (j in seq(1:45))
    if(colnames(region2)[i]==comp[j]){
      shiftforcor2[i]<-shiftst_30[j]
      names(shiftforcor2)[i]<-names(shiftst_30)[j]
    }
}
shkw_r2<-matrix(NA,69,7)
for (i in seq(1,7,1)){
  for (j in seq(1,69,1)){
...
rownames(shkw_r2)<-seq(1908,1976,1)
shkwt_r2<-t(shkw_r2)
...
######region3 24:29   6342200:6731175
region3<-matrix(NA,6,6)
colnames(region3)<-dput(colnames(resw_plot)[24:29])
rownames(region3)<-dput(rownames(resw_plot)[24:29])
for (i in seq(1:6)){
  for(j in seq(1:6)){
    region3[i,j]<-resw_plot[(i+23),(j+23)]
    
  }
}
reg3_cor_y60<-matrix(NA,6,3)
for (i in seq(1:6)){
  for (j in seq(1:69))
...
for (i in seq(1:6)){
  for (j in seq(1:45))
    if(colnames(region3)[i]==comp[j]){
      shiftforcor3[i]<-shiftst_30[j]
      names(shiftforcor3)[i]<-names(shiftst_30)[j]
    }
}
shkw_r3<-matrix(NA,69,6)
for (i in seq(1,6,1)){
  for (j in seq(1,69,1)){
...
rownames(shkw_r3)<-seq(1908,1976,1)
shkwt_r3<-t(shkw_r3)
...
####region4 30:38 6343500:6935400
region4<-matrix(NA,9,9)
colnames(region4)<-dput(colnames(resw_plot)[30:38])
rownames(region4)<-dput(rownames(resw_plot)[30:38])
for (i in seq(1:9)){
  for(j in seq(1:9)){
    region4[i,j]<-resw_plot[(i+29),(j+29)]
    
  }
}
reg4_cor_y60<-matrix(NA,9,3)
for (i in seq(1:9)){
  for (j in seq(1:45))
...
for (i in seq(1:9)){
  for (j in seq(1:45))
    if(colnames(region4)[i]==comp[j]){
      shiftforcor4[i]<-shiftst_30[j]
      names(shiftforcor4)[i]<-names(shiftst_30)[j]
    }
}
shkw_r4<-matrix(NA,69,9)
for (i in seq(1,9,1)){
  for (j in seq(1,69,1)){
...
rownames(shkw_r4)<-seq(1908,1976,1)
shkwt_r4<-t(shkw_r4)
...
#region5 39:45   6340110:6939050
region5<-matrix(NA,7,7)
colnames(region5)<-dput(colnames(resw_plot)[39:45])
rownames(region5)<-dput(rownames(resw_plot)[39:45])
for (i in seq(1:7)){
  for(j in seq(1:7)){
    region5[i,j]<-resw_plot[(i+38),(j+38)]
    
  }
}
reg5_cor_y60<-matrix(NA,7,3)
for (i in seq(1:7)){
  for (j in seq(1:45))
...
for (i in seq(1:7)){
  for (j in seq(1:45))
    if(colnames(region5)[i]==comp[j]){
      shiftforcor5[i]<-shiftst_30[j]
      names(shiftforcor5)[i]<-names(shiftst_30)[j]
    }
}
shkw_r5<-matrix(NA,69,7)
for (i in seq(1,7,1)){
  for (j in seq(1,69,1)){
...
rownames(shkw_r5)<-seq(1908,1976,1)
shkwt_r5<-t(shkw_r5)
...
reg1<-matrix(NA,(16*69*4),1)
for(i in seq(1:16)){
  reg1[c((i*69-68):(i*69)),1]<-region1_shiftst[[i]][,1]
  reg1[c((i*69-68+(16*69)):(i*69+(16*69))),1]<-region1_shiftst[[i]][,2]
  reg1[c((i*69-68+(16*69*2)):(i*69+(16*69*2))),1]<-region1_shiftst[[i]][,3]
  reg1[c((i*69-68+(16*69*3)):(i*69+(16*69*3))),1]<-region1_shiftst[[i]][,4]
}


re1_nam<-matrix(NA,(16*69*4),1)
for(i in seq(1:16)){
  re1_nam[c((i*69-68):(i*69)),1]<-rep(noquote(names(region1_shiftst))[i],69)
  re1_nam[c((i*69-68+(16*69)):(i*69+(16*69))),1]<-rep(noquote(names(region1_shiftst))[i],69)
  re1_nam[c((i*69-68+(16*69*2)):(i*69+(16*69*2))),1]<-rep(noquote(names(region1_shiftst))[i],69)
  re1_nam[c((i*69-68+(16*69*3)):(i*69+(16*69*3))),1]<-rep(noquote(names(region1_shiftst))[i],69)
}
re1_lon<-matrix(NA,(16*69*4),1)
re1_lat<-matrix(NA,(16*69*4),1)
for(i in seq(1:16)){
  re1_lon[c((i*69-68):(i*69)),1]<-rep(reg1_cor_y60[i,2],69)
  re1_lon[c((i*69-68+(16*69)):(i*69+(16*69))),1]<-rep(reg1_cor_y60[i,2],69)
  re1_lon[c((i*69-68+(16*69*2)):(i*69+(16*69*2))),1]<-rep(reg1_cor_y60[i,2],69)
  re1_lon[c((i*69-68+(16*69*3)):(i*69+(16*69*3))),1]<-rep(reg1_cor_y60[i,2],69)
  re1_lat[c((i*69-68):(i*69)),1]<-rep(reg1_cor_y60[i,3],69)
  re1_lat[c((i*69-68+(16*69)):(i*69+(16*69))),1]<-rep(reg1_cor_y60[i,3],69)
  re1_lat[c((i*69-68+(16*69*2)):(i*69+(16*69*2))),1]<-rep(reg1_cor_y60[i,3],69)
  re1_lat[c((i*69-68+(16*69*3)):(i*69+(16*69*3))),1]<-rep(reg1_cor_y60[i,3],69)
}
reg1_shiftst_df<-data.frame(rep(rep(c(1908:1976),16),4),c(rep("wei",(16*69)),rep("gum",(16*69)),
                                                          rep("pe3",(16*69)),rep("ln3",(16*69))),reg1[,1],re1_nam,rep("Region 1",4416),
...
reg2<-matrix(NA,(7*69*4),1) 
for(i in seq(1:7)){
  reg2[c((i*69-68):(i*69)),1]<-region2_shiftst[[i]][,1]
  reg2[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-region2_shiftst[[i]][,2]
  reg2[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-region2_shiftst[[i]][,3]
  reg2[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-region2_shiftst[[i]][,4]
}


re2_nam<-matrix(NA,(7*69*4),1)
for(i in seq(1:7)){
  re2_nam[c((i*69-68):(i*69)),1]<-rep(noquote(names(region2_shiftst))[i],69)
  re2_nam[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(noquote(names(region2_shiftst))[i],69)
  re2_nam[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(noquote(names(region2_shiftst))[i],69)
  re2_nam[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(noquote(names(region2_shiftst))[i],69)
}
re2_lon<-matrix(NA,(7*69*4),1)
re2_lat<-matrix(NA,(7*69*4),1)
for(i in seq(1:7)){
  re2_lon[c((i*69-68):(i*69)),1]<-rep(reg2_cor_y60[i,2],69)
  re2_lon[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(reg2_cor_y60[i,2],69)
  re2_lon[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(reg2_cor_y60[i,2],69)
  re2_lon[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(reg2_cor_y60[i,2],69)
  re2_lat[c((i*69-68):(i*69)),1]<-rep(reg2_cor_y60[i,3],69)
  re2_lat[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(reg2_cor_y60[i,3],69)
  re2_lat[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(reg2_cor_y60[i,3],69)
  re2_lat[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(reg2_cor_y60[i,3],69)
}

reg2_shiftst_df<-data.frame(rep(rep(c(1908:1976),7),4),c(rep("wei",(7*69)),rep("gum",(7*69)),
  rep("pe3",(7*69)),rep("ln3",(7*69))),reg2[,1],re2_nam,rep("Region 2",1932),
...
reg3<-matrix(NA,(6*69*4),1) 
for(i in seq(1:6)){
  reg3[c((i*69-68):(i*69)),1]<-region3_shiftst[[i]][,1]
  reg3[c((i*69-68+(6*69)):(i*69+(6*69))),1]<-region3_shiftst[[i]][,2]
  reg3[c((i*69-68+(6*69*2)):(i*69+(6*69*2))),1]<-region3_shiftst[[i]][,3]
  reg3[c((i*69-68+(6*69*3)):(i*69+(6*69*3))),1]<-region3_shiftst[[i]][,4]
}


re3_nam<-matrix(NA,(6*69*4),1)
for(i in seq(1:6)){
  re3_nam[c((i*69-68):(i*69)),1]<-rep(noquote(names(region3_shiftst))[i],69)
  re3_nam[c((i*69-68+(6*69)):(i*69+(6*69))),1]<-rep(noquote(names(region3_shiftst))[i],69)
  re3_nam[c((i*69-68+(6*69*2)):(i*69+(6*69*2))),1]<-rep(noquote(names(region3_shiftst))[i],69)
  re3_nam[c((i*69-68+(6*69*3)):(i*69+(6*69*3))),1]<-rep(noquote(names(region3_shiftst))[i],69)
}
re3_lon<-matrix(NA,(6*69*4),1)
re3_lat<-matrix(NA,(6*69*4),1)
for(i in seq(1:6)){
  re3_lon[c((i*69-68):(i*69)),1]<-rep(reg3_cor_y60[i,2],69)
  re3_lon[c((i*69-68+(6*69)):(i*69+(6*69))),1]<-rep(reg3_cor_y60[i,2],69)
  re3_lon[c((i*69-68+(6*69*2)):(i*69+(6*69*2))),1]<-rep(reg3_cor_y60[i,2],69)
  re3_lon[c((i*69-68+(6*69*3)):(i*69+(6*69*3))),1]<-rep(reg3_cor_y60[i,2],69)
  re3_lat[c((i*69-68):(i*69)),1]<-rep(reg3_cor_y60[i,3],69)
  re3_lat[c((i*69-68+(6*69)):(i*69+(6*69))),1]<-rep(reg3_cor_y60[i,3],69)
  re3_lat[c((i*69-68+(6*69*2)):(i*69+(6*69*2))),1]<-rep(reg3_cor_y60[i,3],69)
  re3_lat[c((i*69-68+(6*69*3)):(i*69+(6*69*3))),1]<-rep(reg3_cor_y60[i,3],69)
}

reg3_shiftst_df<-data.frame(rep(rep(c(1908:1976),6),4),c(rep("wei",(6*69)),rep("gum",(6*69)),
  rep("pe3",(6*69)),rep("ln3",(6*69))),reg3[,1],re3_nam,rep("Region 3",1656),
...
reg4<-matrix(NA,(9*69*4),1) 
for(i in seq(1:9)){
  reg4[c((i*69-68):(i*69)),1]<-region4_shiftst[[i]][,1]
  reg4[c((i*69-68+(9*69)):(i*69+(9*69))),1]<-region4_shiftst[[i]][,2]
  reg4[c((i*69-68+(9*69*2)):(i*69+(9*69*2))),1]<-region4_shiftst[[i]][,3]
  reg4[c((i*69-68+(9*69*3)):(i*69+(9*69*3))),1]<-region4_shiftst[[i]][,4]
}


re4_nam<-matrix(NA,(9*69*4),1)
for(i in seq(1:9)){
  re4_nam[c((i*69-68):(i*69)),1]<-rep(noquote(names(region4_shiftst))[i],69)
  re4_nam[c((i*69-68+(9*69)):(i*69+(9*69))),1]<-rep(noquote(names(region4_shiftst))[i],69)
  re4_nam[c((i*69-68+(9*69*2)):(i*69+(9*69*2))),1]<-rep(noquote(names(region4_shiftst))[i],69)
  re4_nam[c((i*69-68+(9*69*3)):(i*69+(9*69*3))),1]<-rep(noquote(names(region4_shiftst))[i],69)
}
re4_lon<-matrix(NA,(9*69*4),1)
re4_lat<-matrix(NA,(9*69*4),1)
for(i in seq(1:9)){
  re4_lon[c((i*69-68):(i*69)),1]<-rep(reg4_cor_y60[i,2],69)
  re4_lon[c((i*69-68+(9*69)):(i*69+(9*69))),1]<-rep(reg4_cor_y60[i,2],69)
  re4_lon[c((i*69-68+(9*69*2)):(i*69+(9*69*2))),1]<-rep(reg4_cor_y60[i,2],69)
  re4_lon[c((i*69-68+(9*69*3)):(i*69+(9*69*3))),1]<-rep(reg4_cor_y60[i,2],69)
  re4_lat[c((i*69-68):(i*69)),1]<-rep(reg4_cor_y60[i,3],69)
  re4_lat[c((i*69-68+(9*69)):(i*69+(9*69))),1]<-rep(reg4_cor_y60[i,3],69)
  re4_lat[c((i*69-68+(9*69*2)):(i*69+(9*69*2))),1]<-rep(reg4_cor_y60[i,3],69)
  re4_lat[c((i*69-68+(9*69*3)):(i*69+(9*69*3))),1]<-rep(reg4_cor_y60[i,3],69)
}

reg4_shiftst_df<-data.frame(rep(rep(c(1908:1976),9),4),c(rep("wei",(9*69)),rep("gum",(9*69)),
                                                         rep("pe3",(9*69)),rep("ln3",(9*69))),reg4[,1],re4_nam,rep("Region 4",2484),
...
reg5<-matrix(NA,(7*69*4),1) 
for(i in seq(1:7)){
  reg5 [c((i*69-68):(i*69)),1]<-region5_shiftst[[i]][,1]
  reg5[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-region5_shiftst[[i]][,2]
  reg5[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-region5_shiftst[[i]][,3]
  reg5[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-region5_shiftst[[i]][,4]
}


re5_nam<-matrix(NA,(7*69*4),1)
for(i in seq(1:7)){
  re5_nam[c((i*69-68):(i*69)),1]<-rep(noquote(names(region5_shiftst))[i],69)
  re5_nam[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(noquote(names(region5_shiftst))[i],69)
  re5_nam[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(noquote(names(region5_shiftst))[i],69)
  re5_nam[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(noquote(names(region5_shiftst))[i],69)
}
re5_lon<-matrix(NA,(7*69*4),1) 
re5_lat<-matrix(NA,(7*69*4),1)
for(i in seq(1:7)){
  re5_lon[c((i*69-68):(i*69)),1]<-rep(reg5_cor_y60[i,2],69)
  re5_lon[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(reg5_cor_y60[i,2],69)
  re5_lon[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(reg5_cor_y60[i,2],69)
  re5_lon[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(reg5_cor_y60[i,2],69)
  re5_lat[c((i*69-68):(i*69)),1]<-rep(reg5_cor_y60[i,3],69)
  re5_lat[c((i*69-68+(7*69)):(i*69+(7*69))),1]<-rep(reg5_cor_y60[i,3],69)
  re5_lat[c((i*69-68+(7*69*2)):(i*69+(7*69*2))),1]<-rep(reg5_cor_y60[i,3],69)
  re5_lat[c((i*69-68+(7*69*3)):(i*69+(7*69*3))),1]<-rep(reg5_cor_y60[i,3],69)
}

reg5_shiftst_df<-data.frame(rep(rep(c(1908:1976),7),4),c(rep("wei",(7*69)),rep("gum",(7*69)),
  rep("pe3",(7*69)),rep("ln3",(7*69))),reg5[,1],re5_nam,rep("Region 5",1932),
