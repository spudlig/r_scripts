...
# First: non shifting settings, which means that the user sets a date within the time serie and calculates a HQxxx (default is set to HQ100, but 30,40,50,80,10,150,200 and 300 is implimented as well)
# The start_date can be choosen as soon as the file_name is real and in the working directory, then the time series start and end is printed (after running the function)
...
# Second: shifting settings, which means that one can choose the window range whith which the time window can shift of the whole time series (always the whole series, maybe in the future the possibility for a certain
...
libs <- c('lmom','boot','fitdistrplus','nsRFA','lattice','latticeExtra','agricolae')
...
read_ts<-function(file_name="NONE",start_date="NONE",window_years=30,HQ=100,shift="FALSE",boot="FALSE",alpha=0.05,rep=5000,plot_CI_shift="FALSE",plot_CI="FALSE",GOF="FALSE",Monte_Carlo=2000,...){
  ### get time series
  if(file_name=="NONE"){
  ...
  if(start_date=="NONE"){
    stop(paste("Please selecte a start_date within the following range: ","Time Series Begin: ",years_start," - Time Series End: ",years_end,sep=""))
  }
  nyr<-(nyears_avail-window_years) # get value for matrix length according to window length and total available years. Important for shift
  ...
  seq_wd<-c(as.numeric(rownames(wd_start)):(as.numeric(window_n)+as.numeric(rownames(wd_start)))) # get seq for window
  maxOL<-as.matrix(suppressWarnings(AMS(tab[,2][seq_wd])))
  colnames(maxOL)<-c("Annual Maximum Values")
  ### get AMS for total TS with shift
  maxOLsh<-as.matrix(suppressWarnings(AMS(tab[,2])))
...
### for outside use
file_name="6128101.day"
start_date="1950-01-01"
window_years=30
rep=500
quan=0.99
alpha=0.05
nyears_avail=NROW(tab[,2])/365.25
nyr<-(nyears_avail-window_years)
nyrs<-(window_years)
CI_shift_wei<-matrix(NA,nyr,3)
CI_shift_gum<-matrix(NA,nyr,3)
CI_shift_pe3<-matrix(NA,nyr,3)
